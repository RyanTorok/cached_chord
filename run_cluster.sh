    ...
    for cache in lru none fifo mru lfu lifo; do
        for size in 10 20 50 100 200; do
            ...
            for node in $( seq 0 $(($nodes - 1))); do
                ...
                cmd="killall chord; timeout 500 ~/chord -n $node_id --keys $writes --cache $cache --cache-size $size --master-ip $master_ip --requests $reads --distribution $dist --index $node --total $nodes";
